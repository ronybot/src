...
public class EnglishPennTreebankParseEvaluator<L> {
    ...
    abstract Set<Object> makeObjects(MarinaTree<L> tree);

    public void evaluate(MarinaTree<L> guess, MarinaTree<L> gold) {
     ...
     * with each other.  */
    public void evaluate(MarinaTree<L> guess, MarinaTree<L> gold, PrintWriter pw) {
...
//      gold.pennPrint(pw);
    ...
    private void displayPRF(String prefixStr, int correct, int guessed, int gold, int exact, int total, PrintWriter pw) {
      ...
      String displayStr = " Precision: " + ((int) (precision * 10000)) / 100.0 + " Recall: " + ((int) (recall * 10000)) / 100.0 + " F1: " + ((int) (f1 * 10000)) / 100.0 + " EX: "+((int) (exactMatch * 10000)) / 100.0 ;
  ...
  public static class LabeledConstituentEval<L> extends AbstractEval<L> {
    ...
    static <L> MarinaTree<L> stripLeaves(MarinaTree<L> tree) {
      if (tree.isLeaf())
        return null;
      if (tree.isPreTerminal())
        return new MarinaTree<L>(tree.getLabel());
      List<MarinaTree<L>> children = new ArrayList<MarinaTree<L>>();
      for (MarinaTree<L> child : tree.getChildren()) {
        children.add(stripLeaves(child));
      }
      return new MarinaTree<L>(tree.getLabel(), children);
    }

    Set<Object> makeObjects(MarinaTree<L> tree) {
    	MarinaTree<L> noLeafTree = stripLeaves(tree);
      Set<Object> set = new HashSet<Object>();
      addConstituents(noLeafTree, set, 0);
      return set;
    }

    private int addConstituents(MarinaTree<L> tree, Set<Object> set, int start) {
      if (tree.isLeaf()) {
  ...
  public static void main(String[] args) throws Throwable {
    MarinaTree<String> goldTree = (new Trees.PennTreeReader(new StringReader("(ROOT (S (NP (DT the) (NN can)) (VP (VBD fell))))"))).next();
    MarinaTree<String> guessedTree = (new Trees.PennTreeReader(new StringReader("(ROOT (S (NP (DT the)) (VP (MB can) (VP (VBD fell)))))"))).next();
