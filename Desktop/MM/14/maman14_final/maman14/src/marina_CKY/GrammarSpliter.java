...
public class GrammarSpliter {
	...
	MarinaGrammar newGrammar;
	SimpleLexicon newLexicon;
	final static char LableMark = '^';
	Map<String, List<String>> splitMap;

	public GrammarSpliter(MarinaGrammar grammar, SimpleLexicon lexicon) {
	...
	public MarinaGrammar getNewGrammar() {
		return newGrammar;
	}
	...
	private void buildGrammarVariance() {
		...
		for (String state : newGrammar.getStates()) {
			Utils.addToValueList(stateVariance, getBaseState(state),
					state);
		}
	}
	...
	private void splitOldGrammar(MarinaGrammar oldGrammar) {
		...
		newGrammar = new MarinaGrammar(unaryRuleCounter, binaryRuleCounter, false);
	}
