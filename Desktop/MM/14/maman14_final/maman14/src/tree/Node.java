...
public class Node {
	...
	public Node(String label, List<Node> daughters, Node parent) {
	
		super();
		setIdentifier(label);
		
		for (int i = 0; i < daughters.size(); i++) {
			if (daughters.get(i) instanceof Terminal)
			{
				Terminal td = (Terminal)daughters.get(i);
				Terminal tdc =(Terminal)td.clone();
				addDaughter(tdc);
				tdc.setParent(parent);
				
			}
			else
			{
				Node nd = (Node)daughters.get(i);
